...
> 这份文档只干一件事：把 `main` 当前已经落地、以及 `0.2.x-alpha` 当前明确承诺的边界写清楚。不是规划稿，不掺 P1 / P2 愿景。
...
- `Folio\Core\Foundation\Application` 作为唯一对外 Application facade
...
- PHPUnit、PHP-CS-Fixer、GitHub Actions CI、治理检查
...
结论：**Folio 当前是 `0.2.x-alpha` 可运行内核，不是 `0.1.x` 静态骨架，也不是已经定型的成熟框架。**
